...
- **Compilation sous macOS** : le simulateur compile et passe ses tests sur Apple Silicon. `build.sh` reconnaît Darwin, saute les vérifications X11 et OpenGL qui n'y ont pas de sens, et explique quoi installer plutôt que de proposer une commande apt. Un utilisateur l'a lancé sur un vrai Mac : il tourne.
