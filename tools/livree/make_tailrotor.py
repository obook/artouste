...
Blender importe blade.ac et fournit la geometrie (triangles : coordonnees UV +
position d'envergure de chaque sommet). On rasterise ensuite soi-meme les
triangles de la pale sur une copie de la texture existante, en coloriant chaque
pixel selon sa position d'envergure : metal uni (livree d'origine), ou bandes
jaune/rouge (Gendarmerie). Le moyeu et le disque, hors triangles de la pale, sont
preserves a l'identique.

Produit deux textures :
  - tailrotor.png             : pales metal nu (livree d'origine) ;
  - tailrotor-gendarmerie.png : pales jaunes a zebrures rouges (Gendarmerie).
...
def srgb_vers_lineaire(c):
    """Convertit une couleur sRGB (composantes 0..1) en lineaire (espace des
       pixels Blender). Necessaire pour ecrire la bonne teinte dans l'image."""
...
def metal(_t):
    """Couleur (sRGB) d'un point de la pale en metal nu : gris alliage uni."""
    return (0.66, 0.67, 0.69)


def zebre(t):
    """Couleur (sRGB) d'un point de la pale Gendarmerie selon l'envergure
       normalisee t (0 = pied, 1 = bout) : fond jaune, bandes et bout rouges.
       Positions a affiner sur la photo de reference."""
    jaune = (0.96, 0.80, 0.06)
    rouge = (0.82, 0.07, 0.06)
    bandes_rouges = ((0.50, 0.60), (0.74, 0.82), (0.90, 1.01))  # tip rouge inclus
...
def peindre_pale(px, tris, couleur_fn):
    """Rasterise chaque triangle de la pale dans le tableau de pixels px (H,W,4,
       lineaire, range bas-en-haut comme Blender). La couleur de chaque pixel est
       donnee par couleur_fn(envergure_normalisee)."""
    h, w, _ = px.shape
    ...
    for tri in tris:
        # Coordonnees pixel (u -> x, v -> y depuis le bas, comme Blender).
        ...
        # On colorie pixel par pixel les points interieurs au triangle.
...
def generer(tris, couleur_fn, sortie_png):
    ...
    img.file_format = "PNG"
    img.save()
    print("[tailrotor] ecrit", sortie_png)
...
def main():
    ...
    # 1) Pales metal -> ecrase tailrotor.png (livree d'origine).
    generer(tris, metal, SRC_PNG)
    # 2) Pales zebrees -> tailrotor-gendarmerie.png (Gendarmerie).
