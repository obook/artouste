#!/usr/bin/env python3
# Genere une livree Gendarmerie nationale a partir de l'atlas d'origine de
# l'Alouette II. On repeint en bleu gendarmerie uniquement les pixels neutres
# (le "metal" gris), en laissant intacts les marquages satures (cocardes, orange).
# L'image d'origine n'est jamais modifiee : on ecrit un fichier separe.
import sys
...
# Masque des pixels a repeindre :
#  - le "metal" gris, faiblement sature ;
...
# superieur au bleu (r > g > b), alors que le rouge de la cocarde a g proche de b.
neutral = sat < 0.18
...
# les creux deviennent plus sombres et les hautes lumieres un peu plus claires,
...
print(f"livree ecrite -> {dst}  ({neutral.mean()*100:.0f}% des pixels repeints)")
